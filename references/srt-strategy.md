...
### 3. estimate (`--estimate`, no synthesis)
...
cost/length before paying for synthesis. Also the offline self-test path.
...
(e.g. 蚩尤) are never split across two cards. For CJK set `--max-line-chars` to ~16–20.
